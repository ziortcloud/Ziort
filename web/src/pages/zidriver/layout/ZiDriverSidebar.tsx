...
import {
  User, Calendar, FileText, Search, Briefcase,
...
const SECTIONS = [
  {
    label: 'MY PROFILE',
    items: [
      { label: 'Profile',      icon: User,      to: '/zidriver/profile'      },
      { label: 'Availability', icon: Calendar,  to: '/zidriver/availability' },
      { label: 'Documents',    icon: FileText,  to: '/zidriver/documents'    },
    ],
  },
  {
    label: 'MARKETPLACE',
    items: [
      { label: 'Find Drivers',  icon: Search,   to: '/zidriver/discover'     },
      { label: 'Engagements',   icon: Briefcase,to: '/zidriver/engagements'  },
    ],
  },
]
...
export default function ZiDriverSidebar({ open, onToggle }: Props) {
    ...
    <aside className={`fixed left-0 top-0 h-full bg-orbit-deep border-r border-white/5 flex flex-col transition-all duration-200 z-40 select-none ${open ? 'w-56' : 'w-14'}`}>
      <div className="h-14 flex items-center px-3 border-b border-white/5 shrink-0 gap-2">
        <div className="flex items-center gap-2.5 min-w-0 flex-1 overflow-hidden">
          ...
          {open && <span className="font-bold text-sm text-zi-white whitespace-nowrap"><span className="text-purple-400">Zi</span>Driver</span>}
        </div>
                ...
                <NavLink key={item.to} to={item.to}
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded-md text-[13px] transition-all duration-150
                     ${isActive ? 'bg-orbit-navy text-zi-white border-l-2 border-purple-400' : 'text-zi-muted hover:bg-orbit-navy/50 hover:text-zi-white border-l-2 border-transparent'}`
                  }
              ...
              <div className="absolute bottom-full left-0 mb-1 bg-orbit-navy border border-white/10 rounded-lg p-1.5 grid grid-cols-3 gap-1 shadow-xl z-50 min-w-[90px]">
                {LANGS.map(l => (
                  <button key={l.code} onClick={() => { setLang(l.code); setShowLang(false) }}
                    className={`px-2 py-1 rounded text-[10px] font-medium transition-colors ${lang === l.code ? 'bg-purple-500 text-white' : 'text-zi-muted hover:bg-orbit-deep hover:text-zi-white'}`}>
                    {l.label}
                  </button>
                ))}
              </div>
