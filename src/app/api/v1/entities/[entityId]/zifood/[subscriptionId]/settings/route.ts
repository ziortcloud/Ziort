...
export const PATCH = withErrorHandler(async (req: Request, ctx: any) => {
  ...
  const { entityId, subscriptionId } = await ctx.params
  await requireSubscriptionAccess(session, subscriptionId)
  const parsed = UpsertFoodSettingsSchema.safeParse(await req.json())
  if (!parsed.success) return validationError(parsed.error)
  const { data, error } = await db.from('zfd_settings').upsert({
  ...
  await writeAudit({ action: 'UPDATE', table_name: 'zfd_settings', record_id: data.id,
